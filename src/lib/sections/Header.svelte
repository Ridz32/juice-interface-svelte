<script>
	import Button from '$lib/utils/Button.svelte';
	import DropDown from '$lib/utils/DropDown.svelte';
	import { chainId, connectedAccount, connectWallet, networks } from '$stores/web3';

	let showMoreOptions = false;
</script>

<div class="header">
	<div class="logo">
		<a href="/">
			<span
				style="box-sizing:border-box;display:inline-block;overflow:hidden;width:initial;height:initial;background:none;opacity:1;border:0;margin:0;padding:0;position:relative;max-width:100%"
			>
				<span
					style="box-sizing:border-box;display:block;width:initial;height:initial;background:none;opacity:1;border:0;margin:0;padding:0;max-width:100%"
				>
					<img
						style="display:block;max-width:100%;width:initial;height:initial;background:none;opacity:1;border:0;margin:0;padding:0"
						alt=""
						aria-hidden="true"
						src="data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNTgiIGhlaWdodD0iNTgiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyIgdmVyc2lvbj0iMS4xIi8+"
					/>
				</span><img
					alt="Logo"
					src="/logo.svg"
					decoding="async"
					data-nimg="intrinsic"
					style="position:absolute;top:0;left:0;bottom:0;right:0;box-sizing:border-box;padding:0;border:none;margin:auto;display:block;width:0;height:0;min-width:100%;max-width:100%;min-height:100%;max-height:100%"
				/>
			</span>
		</a>
	</div>
	<div class="actions">
		{#if $connectedAccount}
			{#if networks.ETHEREUM.find((net) => net.chainId === $chainId)}
				<Button on:click={() => ($connectedAccount = '')}>
					Disconnect {$connectedAccount.slice(0, 5)}...{$connectedAccount.slice(-2)}
				</Button>
			{:else}
				<Button on:click={() => alert('Switch to mainnet')}>Switch to Mainnet</Button>
			{/if}
		{:else}
			<Button on:click={() => connectWallet()}>Connect Wallet</Button>
		{/if}
		<span style="position: relative;">
			<Button bgColor="#eee" icon on:click={() => (showMoreOptions = !showMoreOptions)}>
				<img
					src="data:image/png;base64,"
					alt="settings"
				/>
			</Button>
			{#if showMoreOptions}
				<DropDown />
			{/if}
		</span>
	</div>
</div>

<style>
	.header {
		padding: 1.5rem 3rem;
		display: -moz-box;
		display: flex;
		-moz-box-pack: justify;
		justify-content: space-between;
		-webkit-box-pack: justify;
		-moz-box-align: center;
		align-items: center;
	}
	.header > div:nth-child(1),
	.header > div:nth-child(2) {
		width: auto;
		-moz-box-flex: 1;
		flex: 1 1 50%;
		display: -moz-box;
		display: flex;
		-moz-box-align: center;
		align-items: center;
	}
	.header > div:nth-child(2) {
		width: auto;
		-moz-box-flex: 1;
		flex: 1 1 50%;
		display: -moz-box;
		display: flex;
		-moz-box-align: center;
		align-items: center;
	}
	.actions {
		-moz-box-pack: end;
		justify-content: flex-end;
		display: flex;
		gap: 0.5rem;
	}
</style>
