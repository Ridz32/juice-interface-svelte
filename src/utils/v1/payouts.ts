...
import { formatWad, fromWad, parseWad, percentToPerbicent, stripCommas } from '$utils/formatNumber';
import { amountAddFee, amountSubFee } from '$utils/math';

// Returns formatted 'funding target after fee'
// given 'funding target'
export const targetToTargetSubFeeFormatted = (target: string, fee: BigNumber | undefined) => {
...
export function getAmountFromPercent(
	...
	const amount = fromWad(
		amountSubFee(parseWad(stripCommas(target)), percentToPerbicent(feePercentage))
