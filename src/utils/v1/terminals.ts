...
export const getTerminalVersion = (address?: string): V1TerminalVersion | undefined => {
	if (!address) return;

	if (
		address.toLowerCase() ===
		loadTerminalAddress(readNetwork.name, V1ContractName.TerminalV1).toLowerCase()
	) {
		return '1';
	}
	...
	if (
		address.toLowerCase() ===
		loadTerminalAddress(readNetwork.name, V1ContractName.TerminalV1_1).toLowerCase()
	) {
		return '1.1';
	}
};
...
export const getTerminalName = ({
	...
	const _version = version ?? (address ? getTerminalVersion(address) : undefined);
