...
import type { V1FundingCycle, V1FundingCycleMetadata } from '$models/v1/fundingCycle';
...
import type { EditingV1FundingCycle } from './serializers';
...
export const decodeFundingCycleMetadata = (
	...
	const version = metadata.and(bits8).toNumber() as V1FundingCycleMetadata['version'];
	...
	return {
		...
		payIsPaused: version === 0 ? null : Boolean(metadata.shr(32).and(bits1).toNumber()),
		ticketPrintingIsAllowed: version === 0 ? null : Boolean(metadata.shr(33).and(bits1).toNumber()),
...
export const encodeFundingCycleMetadata = (
	...
	if (ticketPrintingIsAllowed !== null) {
		encoded = encoded.or(BigNumber.from(ticketPrintingIsAllowed ? 1 : 0).shl(33));
	}
 ...
 */
export const isRecurring = (fundingCycle: V1FundingCycle | EditingV1FundingCycle) =>
...
export const getUnsafeV1FundingCycleProperties = (
	...
	const reservedRatePercentage = parseFloat(perbicentToPercent(metadata?.reservedRate));
...
export const fundingCycleRiskCount = (fundingCycle: V1FundingCycle): number => {
	return Object.values(getUnsafeV1FundingCycleProperties(fundingCycle)).filter((v) => v === true)
		.length;
};
